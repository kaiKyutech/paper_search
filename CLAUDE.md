...
### 4. 要約機能（2025-06-17改善完了）
- **簡潔要約**: 自動実行、200文字以内の詳細要約（背景・手法・結果を含む）
- **詳細要約**: 構造化された技術詳細分析
  - **常時表示**: キーワード + 背景
  - **展開時表示**: 手法、結果、結論の3項目
  - 簡潔要約を前提とした補完的な構造化情報
...
- **ResultsPage の内部**
  ...
  - 要約機能は `useSummary` フックを通じて `/quick-summary` `/summarize` へアクセスし、`QuickSummary` / `DetailedSummary` コンポーネントでUI表示します。
    - **QuickSummary**: 200文字以内の詳細要約（背景・手法・結果を含む）+ キーワード
    - **DetailedSummary**: 構造化要約（背景は常時表示、手法・結果・結論は展開時表示）
...
### 要約機能の最適化完了
1. **簡潔要約の内容強化**
   - 100文字 → 200文字以内の詳細要約に変更
   - 背景・手法・結果を含む具体的な情報を提供
   - 数値や性能指標を含む技術的詳細を記述

2. **詳細要約のUI/UX改善**
   - 一言要約を削除し、背景を常時表示に変更
   - 手法・結果・結論は展開時のみ表示
   - 簡潔要約との重複を排除し、補完的な構造化情報として機能

3. **LLMプロンプト最適化**
   - 日本語生成の明示的指示を追加
   - 構造化要約は簡潔要約を前提とした設計に変更
   - 技術的詳細に特化した4項目構成（背景・手法・結果・結論）

### ファイル変更点
- `backend/main.py`: 
  - 簡潔要約プロンプト: 100文字→200文字以内の詳細要約に変更
  - 詳細要約プロンプト: 日本語生成指示を追加、what_they_did削除
  - `StructuredSummary`モデル: what_they_didフィールドを削除
- `frontend/features/summary/DetailedSummary.tsx`: 
  - 一言要約セクションを背景セクションに変更
  - 背景を常時表示、手法・結果・結論は展開時のみ表示
