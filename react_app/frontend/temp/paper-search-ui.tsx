...
import { Search, Menu, FileText, Settings, User, Clock, Bookmark, TrendingUp, X, Upload, File, ChevronDown } from 'lucide-react';

const PaperSearchApp = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isSearched, setIsSearched] = useState(false);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [uploadedFiles, setUploadedFiles] = useState([]);
  const [searchType, setSearchType] = useState('keyword'); // 'keyword' or 'ai'
  const [searchEngine, setSearchEngine] = useState('semantic'); // 'semantic' or 'google'
  const [sortOption, setSortOption] = useState('relevance'); // 'relevance', 'date', 'citations'
